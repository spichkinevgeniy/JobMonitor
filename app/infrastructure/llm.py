...
def get_resume_parse_agent() -> Agent[None, OutResumeParse]:
    ...
    system_prompt = (
        ...
        "кандидата по его должностям и опыту. Если человек по факту НЕ разработчик/инженер "
        "(например дизайнер, маркетолог, менеджер, аналитик без кода, HR), specializations и "
        "skills должны остаться ПУСТЫМИ — даже если в тексте упоминаются технические слова или "
        "платформы (iOS, Android, React и т.п.) в контексте его нетехнической работы "
        "(дизайн интерфейса, управление проектом и т.д. — это не значит, что он их разрабатывал).\n"
